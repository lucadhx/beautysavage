...
function renderPractitionerListCard(adminAccount) {
  ...
  return `
    <article class="svm-pract-card">
      <div class="svm-pract-card__name">${escHtml(formatPersonName(adminAccount))}</div>
...
function getDefaultForm() {
  return {
    ...
    bufferTime: 0, cancellationDays: 7, options: [],
    photos: [],
...
function renderTarifsTab() {
  ...
  return `
    ...
    ` : ''}
    <div class="svm-field">
      <label>D̩lai annulation (jours)</label>
...
export async function renderModule(container) {
  ...
  await Promise.all([loadServices(), loadPractitioners(), loadMyPractitionerProfile()]);
